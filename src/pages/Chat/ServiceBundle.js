...
import { getListConversationQueues, getListConversationQueueByUser } from "services/api/conversationQueue"
  ...
  const fetchData = useCallback(
    async ({ pageSize, pageIndex }) => {
      ...
      if (fetchId === fetchIdRef.current) {
        try {
          setLoading(true)
          const res = await getListConversationQueueByUser(currentUser.id,
            {
              pageSize: 10,
              page: pageIndex + 1,
            },);
