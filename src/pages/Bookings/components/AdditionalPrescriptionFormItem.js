...
const PrescriptionFormItem = ({
  ...
  const [additionalDrugs, setAdditionalDrugs] = useState();
  const [listDrugs, setListDrugs] = useState(allDrugs)
  const [loading, setLoading] = useState(false)

  useEffect(() => {
    let drugs = allDrugs.filter(d => d.type == "Hàng hoá khác");
    setAdditionalDrugs(drugs);
    setListDrugs(drugs)
  }, [allDrugs])

  const handleSearchDrugs = (filter) => {
    const drugs = additionalDrugs.filter(d => d.label_i?.toLowerCase().includes(filter) || d.ingredient_i?.toLowerCase().includes(filter));
  ...
  return (
    <div className="flex flex-col gap-2" key={item.id}>
      <div className="grid grid-cols-7 gap-4">
        <Controller
          ...
          render={({ field: { onChange, value, ref } }) => (
            <Select
              ...
              onChange={(e) => {
                console.log('e', e)
                setValue(`additional_drugs[${index}].drug`, e, {
                  shouldValidate: true,
                  shouldDirty: true,
                })
                setValue(`additional_drugs[${index}].unit`, e.unit, {
                  shouldValidate: true,
                  shouldDirty: true,
                })
              }}
        ...
        />
        <Controller
          name={`additional_drugs[${index}].morningAmount`}
          control={control}
        ...
        />
        <Controller
          name={`additional_drugs[${index}].afternoonAmount`}
          control={control}
          render={({ field: { onChange, value } }) => (
            <Input
              type="text"
              inputClassName="flex-1 "
              name={`additional_drugs[${index}].noonAmount`}
              onChange={(e) => {
                onChange(e)
                handleUpdateAmount()
              }}
        ...
        />
        <Controller
          name={`additional_drugs[${index}].eveningAmount`}
          control={control}
          render={({ field: { onChange, value } }) => (
            <Input
              type="text"
        ...
        />
        <Controller
          name={`additional_drugs[${index}].numberOfDays`}
          control={control}
          render={({ field: { onChange, value } }) => (
            <Input
              type="text"
        ...
        />
        <Controller
          name={`additional_drugs[${index}].amount`}
          control={control}
          render={({ field: { onChange, value } }) => (
            <Input
              type="text"
        ...
        />
        <Controller
          name={`additional_drugs[${index}].unit`}
          control={control}
          render={({ field: { onChange, value } }) => (
            <Input
              inputClassName="flex-1 "
